...
public final class ShulkerBoxTooltip implements ModInitializer, ShulkerBoxTooltipApi {
  ...
  public static void initPreviewItemsMap() {
    if (!registeredProviders) {
      ...
      LOGGER.info("[" + MOD_NAME + "] " + pluginText, plugins.size(), plugins.stream()
          .map(p -> p.getProvider().getMetadata().getId()).collect(Collectors.joining(", ")));
      for (EntrypointContainer<ShulkerBoxTooltipApi> container : plugins) {
        ...
        LOGGER.info("[" + MOD_NAME + "] " + providerText, registered,
